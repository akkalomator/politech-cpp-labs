...
Shape getShapeFromLine(const Line &line)
{
  ...
  if (std::find_if(remLine.begin(), remLine.end(),
                   [](char c)
                   { return !std::isspace(c, std::locale()); }) != remLine.end())
  {
...
bool isSquare(const Shape &shape)
{
  ...
  return std::any_of(distances.begin(), distances.end(), [hyp](int d)
  { return (2 * d == hyp); });
}
...
{
  out << shape.size();
  std::for_each(shape.begin(), shape.end(), [&out](const Point &p)
  { printPoint(p, out); });
  out << "\n";
}
...
void task2(std::istream &in, std::ostream &out)
{
  ...
  shapes.erase(std::remove_if(shapes.begin(), shapes.end(),
                              [](const Shape &sh)
                              { return sh.size() == 5; }), shapes.end());

  std::vector<Point> points;
  std::transform(shapes.begin(), shapes.end(),
                 std::back_inserter(points), [](const Shape &sh)
                 { return sh.front(); });

  out << "Points:";
  std::for_each(points.begin(), points.end(), [&out](const Point &p)
  { printPoint(p, out); });
  out << std::endl;
  ...
  out << "Shapes:" << std::endl;
  std::for_each(shapes.begin(), shapes.end(), [&out](const Shape &sh)
  { printShape(sh, out); });
}
